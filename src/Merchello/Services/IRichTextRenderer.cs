...
public interface IRichTextRenderer
{
    ...
    /// <summary>
    /// Asynchronously renders a RichTextEditorValue JSON string to HTML with resolved links, media, and blocks.
    /// Prefer this method over <see cref="Render"/> when calling from async contexts.
    /// </summary>
    /// <param name="richTextJson">The JSON string containing markup and optional blocks</param>
    /// <returns>HTML-encoded string safe for Razor rendering</returns>
    Task<IHtmlEncodedString> RenderAsync(string? richTextJson);
}
