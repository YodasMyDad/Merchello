import { html as t, nothing as n, css as g, state as r, customElement as v } from "@umbraco-cms/backoffice/external/lit";
...
var y = Object.defineProperty, x = Object.getOwnPropertyDescriptor, a = (i, e, o, p) => {
  for (var u = p > 1 ? void 0 : p ? x(e, o) : e, c = i.length - 1, l; c >= 0; c--)
    (l = i[c]) && (u = (p ? l(e, o, u) : l(u)) || u);
  return p && u && y(e, o, u), u;
};
const h = "__no-shipping__", m = "No Shipping";
let s = class extends f {
  ...
  async _loadWarehouses() {
    ...
    try {
      const { data: i, error: e } = await _.getWarehousesList();
  ...
  async _loadShippingOptions() {
    ...
    try {
      const { data: e, error: o } = await _.getShippingOptionsForWarehouse(
  ...
  _submitItem() {
    const i = this._warehouses.find((p) => p.id === this._selectedWarehouseId), e = this._shippingOptions.find((p) => p.id === this._selectedShippingOptionId), o = this._selectedShippingOptionId === h;
    this.value = {
      item: {
        ...
        shippingOptionId: this._isPhysicalProduct ? o ? null : this._selectedShippingOptionId ?? void 0 : void 0,
        shippingOptionName: this._isPhysicalProduct ? o ? m : e?.name ?? void 0 : void 0
      }
  ...
  _refreshTaxPreview() {
    this._taxPreviewDebounceTimer && clearTimeout(this._taxPreviewDebounceTimer), this._taxPreviewDebounceTimer = setTimeout(async () => {
      ...
      try {
        const { data: i, error: e } = await _.previewCustomItemTax({
          price: this._price,
          quantity: this._quantity,
          taxGroupId: this._selectedTaxGroupId
        });
  ...
  _renderDetailsStep() {
    const i = this.data?.currencySymbol ?? "£", e = this._taxPreview !== null, o = this._taxPreview?.taxRate, p = this._taxPreview?.subtotal, u = this._taxPreview?.taxAmount, c = this._taxPreview?.total;
    return t`
      <div class="form-row-group">
        ...
        <div class="form-row">
          ...
          <uui-input
            id="item-sku"
            .value=${this._sku}
            @input=${(l) => this._sku = l.target.value}
        ...
        <div class="form-row">
          ...
          <div class="input-with-prefix">
            ...
            <uui-input
              ...
              @input=${(l) => this._cost = parseFloat(l.target.value) || 0}
              step="0.01"
              min="0"
            ></uui-input>
          </div>
      ...
      <div class="form-row">
        ...
        ${this._selectedTaxGroupId && this._price > 0 ? t`
          <span class="tax-info">
            ${e && u !== void 0 && o !== void 0 ? `Tax: ${i}${d(u, 2)} at ${o}%` : t`<span class="calculating">Calculating tax...</span>`}
          </span>
        ` : n}
      </div>
      ...
      <div class="form-row checkbox-row">
        <uui-checkbox
          ...
          @change=${(l) => this._isPhysicalProduct = l.target.checked}
        >
      ...
      ${this._price > 0 ? t`
        <div class="summary ${this._isLoadingTaxPreview || !e ? "loading" : ""}">
          ...
          ${this._selectedTaxGroupId ? t`
            <div class="summary-row">
              <span>Tax${e && o !== void 0 ? ` (${o}%)` : ""}</span>
              <span>
                ${e && u !== void 0 ? `${i}${d(u, 2)}` : t`<span class="calculating">...</span>`}
              </span>
            </div>
          ` : n}
  ...
  _renderSelectedShippingDetails() {
    if (this._selectedShippingOptionId === h) return n;
    const i = this._shippingOptions.find((o) => o.id === this._selectedShippingOptionId);
    if (!i) return n;
  ...
  _getShippingOptionOptions() {
    ...
    return [
      ...
      })),
      {
        name: m,
        value: h,
        selected: this._selectedShippingOptionId === h
      }
    ];
  }
  _renderActions() {
    const i = this._step === "details", e = i && this._isPhysicalProduct, o = !i || !this._isPhysicalProduct;
    return t`
      <div slot="actions">
        ${!i ? t`
...
s = a([
  v("merchello-add-custom-item-modal")
], s);
const P = s;
export {
...
//# sourceMappingURL=add-custom-item-modal.element-DHygbEFs.js.map
