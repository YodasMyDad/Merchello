import { LitElement as C, html as s, nothing as c, css as P, property as S, state as p, customElement as A } from "@umbraco-cms/backoffice/external/lit";
import { UmbModalBaseElement as T } from "@umbraco-cms/backoffice/modal";
import { M as w } from "./merchello-api-Fd1MNOAp.js";
import { c as L } from "./formatting-Wns_rJIa.js";
...
let u = class extends T {
  ...
  async _mapToPickerVariant(e, t) {
    ...
    if (m) {
      ...
      d && (b = d.stockStatus, R = d.stockStatusLabel, I = d.stockStatusCssClass);
    }
    return {
      id: e.id,
      productRootId: t.id,
  ...
  _renderAddonSelectionView() {
    const e = this._pendingAddonSelection;
    if (!e) return c;
    const t = e.variant, i = t.optionValuesDisplay ? `${t.rootName} - ${t.optionValuesDisplay}` : t.rootName, a = this._addonPricePreview, o = this._addonPreviewError !== null, r = a?.basePrice ?? t.price, n = a?.addonsTotal ?? 0, m = a?.totalPrice ?? t.price, g = this._getMissingRequiredAddonOptions(), l = g.length > 0, f = e.addonOptions.some((b) => b.isRequired), x = !o && !this._isLoadingAddonPreview && !l;
    return s`
      <umb-body-layout headline="${f ? "Select Add-ons" : "Select Add-ons (Optional)"}">
        <div id="main">
          ...
          <div class="addon-options">
            ${e.addonOptions.map((b) => this._renderAddonOption(b))}
          </div>
        </div>
...
//# sourceMappingURL=product-picker-modal.element-y9fHTPtN.js.map
