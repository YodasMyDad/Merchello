...
async function t(e) {
  try {
    ...
    let g;
    return n.includes("application/json") ? g = await s.json() : g = await s.text(), { data: g };
  } catch (r) {
...
async function o(e, r) {
  try {
    const i = await h(), s = p.baseUrl || "", n = await fetch(`${s}${u}/${e}`, {
      method: "POST",
      credentials: p.credentials,
      headers: i,
...
async function d(e) {
  try {
    ...
    if (!s.ok) {
      ...
      return { error: new Error(n || `HTTP ${s.status}: ${s.statusText}`) };
    }
    return {};
  } catch (r) {
    return { error: r instanceof Error ? r : new Error(String(r)) };
...
const v = {
  ...
  updateSupplier: (e, r) => a(`suppliers/${e}`, r),
  /** Delete a supplier */
...
//# sourceMappingURL=merchello-api-chPgfVAp.js.map
