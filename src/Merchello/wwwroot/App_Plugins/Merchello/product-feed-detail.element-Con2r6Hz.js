    ...
    return o`
            ...
            <div class="query-groups">
              <section class="query-group">
                ...
                <div class="query-tag-list">
                  ${x.productTypeNames.length > 0 ? x.productTypeNames.map((m) => o`<uui-tag color="warning">${m}</uui-tag>`) : o`<uui-tag color="positive">Any</uui-tag>`}
                </div>
              </section>
              ...
              <section class="query-group">
                ...
                <div class="query-tag-list">
                  ${x.collectionNames.length > 0 ? x.collectionNames.map((m) => o`<uui-tag color="warning">${m}</uui-tag>`) : o`<uui-tag color="positive">Any</uui-tag>`}
                </div>
              </section>
                ...
                ${x.filterGroups.length === 0 ? o`
                    ...
                    ` : o`
                      <div class="query-filter-groups">
                        ${x.filterGroups.map((m) => o`
                          <div class="query-filter-group">
                            <span class="query-filter-group-name">${m.groupName}</span>
                            <div class="query-tag-list">
                              ${m.valueNames.map((S) => o`<uui-tag color="warning">${S}</uui-tag>`)}
                            </div>
                          </div>
                        `)}
                      </div>
      ...
      <uui-box headline="Product Types">
        ...
        ${this._productTypes.length === 0 ? o`<p class="hint">No product types found.</p>` : o`
              <div class="checkbox-grid">
                ${this._productTypes.map((m) => {
      const S = e.productTypeIds.includes(m.id);
      return o`
                    <uui-checkbox
                      ?checked=${S}
                      @change=${(A) => t(m.id, A.target.checked)}>
                      ${m.name}
                    </uui-checkbox>
                  `;
    })}
      ...
      <uui-box headline="Collections">
        ...
        ${this._collections.length === 0 ? o`<p class="hint">No collections found.</p>` : o`
              <div class="checkbox-grid">
                ${this._collections.map((m) => {
      const S = e.collectionIds.includes(m.id);
      return o`
                    <uui-checkbox
                      ?checked=${S}
                      @change=${(A) => i(m.id, A.target.checked)}>
                      ${m.name}
                    </uui-checkbox>
                  `;
    })}
      ...
      <uui-box headline="Filter Values">
        ...
        ${this._filterGroups.length === 0 ? o`<p class="hint">No filter groups found.</p>` : o`
              <div class="group-list">
                ${this._filterGroups.map((m) => {
      const S = e.filterValueGroups.find((P) => P.filterGroupId === m.id), A = this._getSelectedFilterCount(e, m.id), R = (m.filters ?? []).length, G = R > 0 && A === R, q = A === 0 ? `0 of ${R} selected • Group ignored` : `${A} of ${R} selected • OR in this group`;
      return o`
                    <section class="group-card">
                      ...
                      <div class="checkbox-grid">
                        ${(m.filters ?? []).map((P) => {
        const U = S?.filterIds.includes(P.id) ?? !1;
        return o`
                            <uui-checkbox
                              ?checked=${U}
                              @change=${(V) => r(m.id, P.id, V.target.checked)}>
                              ${P.name}
                            </uui-checkbox>
                          `;
      })}
...
//# sourceMappingURL=product-feed-detail.element-Con2r6Hz.js.map
