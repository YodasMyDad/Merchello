    ...
    ILogger<OrderService> logger) : IOrderService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var shippingOptions = await dbContext.ShippingOptions
        ...
        foreach (var group in shippingResult.WarehouseGroups)
        {
            ...
            if (checkoutSession.SelectedDeliveryDates.TryGetValue(group.GroupId, out var selectedDate))
            {
                ...
                var lineItemsForSurcharge = new List<LineItem>();
            ...
            var orderLineItems = new List<LineItem>();
            foreach (var shippingLineItem in group.LineItems)
            {
                var basketLineItem = basket.LineItems.FirstOrDefault(li => li.Id == shippingLineItem.LineItemId);
                if (basketLineItem == null)
        ...
        try
        {
            ...
            {
                var reservationResults = new List<string>();
                ...
                if (hasUntrackedItems && !(order.LineItems ?? []).Any(li => li.ProductId.HasValue &&
                    inventoryService.IsStockTrackedAsync(li.ProductId.Value, order.WarehouseId, cancellationToken).Result))
                {
            ...
            logger.LogInformation("Created invoice {InvoiceId} with {OrderCount} orders from {GroupCount} warehouse groups",
                invoice.Id, orders.Count, shippingResult.WarehouseGroups.Count);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var lineItemsToShip = order.LineItems?.ToList() ?? new List<LineItem>();
        ...
        // Group by warehouse
        var warehouseGroups = new Dictionary<Guid, List<LineItem>>();
        ...
        foreach (var lineItem in lineItemsToShip.Where(li => li.ProductId.HasValue))
        {
            ...
            {
                warehouseGroups[warehouseId] = new List<LineItem>();
            }
        ...
        var nowShippedQuantity = shipments.SelectMany(s => s.LineItems ?? []).Sum(li => li.Quantity);
        ...
        logger.LogInformation("Created {ShipmentCount} shipments for order {OrderId}. Order status: {Status}",
            shipments.Count, order.Id, order.Status);
