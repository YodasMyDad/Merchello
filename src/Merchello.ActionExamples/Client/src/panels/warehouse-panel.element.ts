...
@customElement("merchello-warehouse-panel")
export class MerchelloWarehousePanelElement extends UmbElementMixin(LitElement) {
  @property({ type: String }) warehouseId = "";
  @property({ type: String }) actionKey = "";
  @property({ attribute: false }) closeModal: (() => void) | null = null;

  @state() private _data: WarehouseData | null = null;
  ...
  private async _fetchData(): Promise<void> {
    try {
      ...
      const res = await fetch(`${this.#baseUrl}/umbraco/api/v1/warehouses/${this.warehouseId}`, {
  ...
  override render() {
    ...
    return html`
      <uui-box headline="Warehouse Details">
        ...
        <div class="reference">
          <p>Warehouse ID: ${this.warehouseId}</p>
...
declare global {
  interface HTMLElementTagNameMap {
    "merchello-warehouse-panel": MerchelloWarehousePanelElement;
  }
}
